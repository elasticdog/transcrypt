"""
Ported from cibuildwheel: https://github.com/pypa/cibuildwheel/blob/main/cibuildwheel/oci_container.py
"""
...
from typing import IO, Dict, Sequence, cast, Any
Literal, PathOrStr, PopenBytes = Any, Any, Any
ContainerEngine = Any
...
class OCIContainer:
    ...
    Args:
        volumes (None | List[Tuple[PathLike, PathLike]]):
            A list of (external-path, internal-path) tuples
    ...
    def start(self):
        self.name = f"OCIContainer-{uuid.uuid4()}"
        network_args = []
        shell_args = ["linux32", "/bin/bash"] if self.simulate_32_bit else ["/bin/bash"]

        volume_argvals = ['{}:{}'.format(v1, v2) for v1, v2 in self.volumes]
        volume_argkeys = ['--volume'] * len(volume_argvals)
        volume_args = list(itertools.chain(*zip(volume_argkeys, volume_argvals)))
        print(f'volume_args={volume_args}')
